...
emitter::~emitter() {
    Detener(); // Garantiza la liberación de /dev/video0 al destruir el objeto
}
...
bool emitter::Iniciar() {
    ...
    std::cout << "[emitter] Transmisión asíncrona iniciada hacia " << ip_destino << ":" << puerto << std::endl;
    return true;
}

void emitter::EjecutarBucle() {
    // Este código corre en paralelo. Se bloquea aquí, pero no afecta al robot.
